...
// s3r-llm: the KV cache goes to PSRAM if there is any; at the full 256
// positions it is far too big for internal RAM.
static void* kv_alloc(size_t n) {
...
bool llm_init_embedded(Transformer* T, const uint8_t* model_bytes, size_t model_size,
  ...
  if (p->arch == ARCH_GPTNEO) {
    ...
    s->key_cache4   = (uint8_t*)  kv_alloc(nib_n);
    s->value_cache4 = (uint8_t*)  kv_alloc(nib_n);
    s->k_gscales    = (uint16_t*) kv_alloc(gscale_n * sizeof(uint16_t));
    s->v_gscales    = (uint16_t*) kv_alloc(gscale_n * sizeof(uint16_t));
